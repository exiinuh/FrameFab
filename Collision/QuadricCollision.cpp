...
int	QuadricCollision::ColFreeAngle(vector<lld> StateMap)
{
	int colfree_angle = 0;
	for (int j = 0; j < 3; j++)
	{
	...
	lld mask = (1 << 60);
	if (!((mask& StateMap[2]) >> 60))
	{
		colfree_angle++;
	}
	 mask = (1 << 61);
	if (!((mask& StateMap[2]) >> 61))
	{
		colfree_angle++;
	}
...
void QuadricCollision::Angle(vector<lld> &state_map)
{
	state_map.resize(3);
	for (int i = 0; i < 3; i++)
	{
		state_map[i] = state_map_[i];
	}
}

void QuadricCollision::ModifyAngle(vector<lld> &state_map)
{
	for (int i = 0; i < 3; i++)
	{
		state_map[i] |= state_map_[i];
